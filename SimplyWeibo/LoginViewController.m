    ...
    if (indexPath.section >=1) {
        ...
        return cell;
         ...
         if ([user.userDescription isEqualToString:@""]) {
              cell.introduceLable.text=[NSString stringWithFormat:@"基本介绍 : 暂无介绍"];
         }else{
              cell.introduceLable.text=[NSString stringWithFormat:@"简介: %@",user.userDescription];
         }
...
-(void)initData{
    dataSource = @[@[@"新的好友",@"微博等级"],@[@"我的相册",@"我的点评",@"我的赞"],@[@"微博支付",@"微博运动"],@[@"粉丝头条",@"粉丝服务"],@[@"草稿箱",@"更多"]];
     imageArray = @[
                    ...
                    @[[UIImage imageNamed:@"draft"],[UIImage imageNamed:@"more"]]];
...
-(void)attentionButtonAction:(UIButton *)sender{
     ...
     WebViewViewController *wVC = [WebViewViewController new];
