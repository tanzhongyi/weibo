...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"accessToken"]) {
        _tabBarController.viewControllers = @[_homeVc,_issueVc,_loginVc];
    }else{
        _tabBarController.viewControllers = @[_homeVc,_issueVc,_myVc];
    }
    NSArray *names = @[@"首页",@"发布",@"我"];
    NSArray *images = @[[UIImage imageNamed:@"home"],[UIImage imageNamed:@"arrow-up"],[UIImage imageNamed:@"user"]];
    for (int i = 0; i < 3; i++) {
        _tabBarController.viewControllers[i].tabBarItem.title = names[i];
        _tabBarController.viewControllers[i].tabBarItem.image = images[i];
        if (i == 1) {
            _tabBarController.viewControllers[1].tabBarItem.imageInsets = UIEdgeInsetsMake(6, 0, -6, 0);
            _tabBarController.viewControllers[1].tabBarItem.title = nil;
        }
    }
    _tabBarController.selectedIndex = 2;
    self.window.rootViewController = _tabBarController;
    _tabBarController.tabBar.tintColor = RGBColor(252, 119, 33);
...
-(void)didReceiveWeiboResponse:(WBBaseResponse *)response{
    ...
    if ([response isMemberOfClass:[WBAuthorizeResponse class]]) {
        ...
        [MBProgressHUD hideHUDForView:_tabBarController.viewControllers[2].view animated:YES];
        if (response.statusCode == WeiboSDKResponseStatusCodeSuccess) {
             _loginVc = [[UINavigationController alloc]initWithRootViewController:[LoginViewController new]];

            _tabBarController.viewControllers = @[_homeVc,_issueVc,_loginVc];
            _tabBarController.viewControllers[2].tabBarItem.title = @"我";
            _tabBarController.viewControllers[2].tabBarItem.image = [UIImage imageNamed:@"user"];
